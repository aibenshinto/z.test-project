...
// ---------------------------------------------------------------------------
// openApplication (Part 4 + Part 7)
// ---------------------------------------------------------------------------

test("openApplication verifies the application UI actually opened", async () => {
  const { e } = envWithModel([]);
  ...
  btn.addEventListener("click", () => {
    dialogOpen = true;
    e.make("div", { role: "dialog", rect: { width: 400, height: 300 } });
  });
  ...
  const result = await e.sandbox.__autoApplyAgentLoopCore.openApplication({
    snapshot, opened: () => dialogOpen, settleMax: 300,
  });

  assert.equal(result.opened, true);
});

test("openApplication reports failure when the click is accepted but nothing opens", async () => {
  const { e } = envWithModel([]);
  e.make("button", { text: "Easy Apply", rect: { x: 0, y: 0, width: 120, height: 40 } });

  const snapshot = e.sandbox.__autoApplyObserverCore.buildSnapshot({});
  const result = await e.sandbox.__autoApplyAgentLoopCore.openApplication({
    snapshot, opened: () => false, settleMax: 200,
  });
...
test("openApplication falls back to an observed candidate when the platform hint misses", async () => {
  const { e } = envWithModel([]);
  const btn = e.make("div", { role: "button", "aria-label": "Start application", rect: { x: 0, y: 0, width: 160, height: 44 } });

  let opened = false;
  btn.addEventListener("click", () => { opened = true; });

  const snapshot = e.sandbox.__autoApplyObserverCore.buildSnapshot({});
  const result = await e.sandbox.__autoApplyAgentLoopCore.openApplication({
    hint: () => null, // the platform selector no longer matches anything
    snapshot,
    opened: () => opened,
    settleMax: 300,
  });
...
test("openApplication reports when there is no apply control at all", async () => {
  const { e } = envWithModel([]);
  e.make("button", { text: "Sign in", rect: { width: 100, height: 40 } });
